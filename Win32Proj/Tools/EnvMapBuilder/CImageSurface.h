...
public:
   ...
   // visit one texel([0, size-1])
   CP_ITYPE *GetSurfaceTexelPtr(int32 a_U, int32 a_V);
   CP_ITYPE *GetSafeSurfaceTexelPtr(int32 a_U, int32 a_V);

   // uv [0, 1]
   void GetSurfaceTexelLinearVal( float u, float v, CP_ITYPE* vals ) const;
   ...
   void SetChannelData(int32 a_ChannelIdx, int32 a_SrcPitch, int32 a_srcChannels, const float *a_SrcDataPtr);
   ...
   void Resize( int32 a_Width, int32 a_Height );
   void CopyTo( CImageSurface* destFace );

private:
    //fatal error
    void _FatalError(WCHAR *a_Msg);
};
