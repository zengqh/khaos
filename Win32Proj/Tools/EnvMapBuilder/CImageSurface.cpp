...
void CImageSurface::Init(int32 a_Width, int32 a_Height, int32 a_NumChannels )
{
   ...
   //try 
   {
      ...
      int bytes = sizeof(CP_ITYPE) * m_Width * m_Height * m_NumChannels;
      memset( m_ImgData, 0, bytes );
   }
   //catch ( ... )
   {
...
{
    assert( 0 <= u && u < m_Width );
    assert( 0 <= v && v < m_Height );

    return( m_ImgData + (((m_Width * v) + u) * m_NumChannels) );
}

CP_ITYPE * CImageSurface::GetSafeSurfaceTexelPtr(int32 a_U, int32 a_V)
{
    int lastW = m_Width - 1;
    int lastH = m_Height - 1;

    VM_CLAMP(a_U, a_U, 0, lastW);
    VM_CLAMP(a_V, a_V, 0, lastH);

    return( m_ImgData + (((m_Width * a_V) + a_U) * m_NumChannels) );
}

void CImageSurface::GetSurfaceTexelLinearVal( float u, float v, CP_ITYPE* vals ) const
{
    // [0, size]
    float u_pix = u * m_Width;
    float v_pix = v * m_Height;
    ...
    // low high uv int
    int lowx = (int)floor( u_pix );
    int lowy = (int)floor( v_pix );
    ...
    // data
    CImageSurface* pThis = const_cast<CImageSurface*>(this);
    float* valA = pThis->GetSafeSurfaceTexelPtr(lowx, lowy);
    float* valB = pThis->GetSafeSurfaceTexelPtr(highx, lowy);
    float* valC = pThis->GetSafeSurfaceTexelPtr(lowx, highy);
    float* valD = pThis->GetSafeSurfaceTexelPtr(highx, highy);
    ...
    VM_LERP_N( vals, valAB, valCD, channels, ay ); // 再垂直插值
}
...
void CImageSurface::SetChannelData(int32 a_ChannelIdx, int32 a_SrcPitch, int32 a_srcChannels, 
                                   const float *a_SrcDataPtr)
{
    int32 u, v;
    CP_ITYPE *texelPtr;
    ...
    for(v=0; v<m_Height; v++)
    {
        const float* lineData = (float*)((char*)a_SrcDataPtr + a_SrcPitch * v);

        for(u=0; u<m_Width; u++)
        {
            texelPtr = GetSurfaceTexelPtr( u, v );

            *(texelPtr + a_ChannelIdx) = *(lineData + u * a_srcChannels);
        }    
    }
}
...
void CImageSurface::Resize( int32 a_Width, int32 a_Height )
{
    if ( a_Width == m_Width && a_Height == m_Height )
        return;

    CP_ITYPE* newImgData = new CP_ITYPE[a_Width * a_Height * m_NumChannels];

    for ( int j = 0; j < a_Height; ++j )
    {
        for ( int i = 0; i < a_Width; ++i )
        {
            float u = (i + 0.5f) / a_Width;
            float v = (j + 0.5f) / a_Height;

            CP_ITYPE* dest = newImgData + ((a_Width * j + i) * m_NumChannels);

            this->GetSurfaceTexelLinearVal( u, v, dest );
        }
    }

    // 交换内存
    VM_SWAP(m_ImgData, newImgData);
    m_Width = a_Width;
    m_Height = a_Height;

    // 释放不用的
    SAFE_DELETE_ARRAY(newImgData);    
}

void CImageSurface::CopyTo( CImageSurface* destFace )
{
    assert( m_NumChannels == destFace->m_NumChannels );

    int destWidth  = destFace->m_Width;
    int destHeight = destFace->m_Height;

    for ( int j = 0; j < destHeight; ++j )
    {
        for ( int i = 0; i < destWidth; ++i )
        {
            float u = (i + 0.5f) / destWidth;
            float v = (j + 0.5f) / destHeight;

            CP_ITYPE* dest = destFace->m_ImgData + ((destWidth * j + i) * destFace->m_NumChannels);
...
void CImageSurface::WriteHDRFile(WCHAR *a_FileName )
{
   ...
   if(m_NumChannels != 3)
   {    
      _FatalError(L"CImageSurface::WriteHDRFile: only works for 3 channel CImageSurfaces.");
      return;
   }

   if(sizeof(CP_ITYPE) != 4)
   {    
      _FatalError(L"CImageSurface::WriteHDRFile: requires internal data format CP_ITYPE to be float32");
      return;
   }
...
//--------------------------------------------------------------------------------------
void CImageSurface::_FatalError(WCHAR *a_Msg)
{
